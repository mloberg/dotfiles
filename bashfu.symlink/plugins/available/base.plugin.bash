...
pmdown () {
	...
	if command -v markdown &>/dev/null; then
		rm -f /tmp/pmdown.html
		markdown $1 > /tmp/pmdown.html
		open /tmp/pmdown.html
	else
		echo "You don't have a markdown command installed! (brew install markdown)"
	fi
}
...
find_in_files() {
	about 'search files for a text string'
